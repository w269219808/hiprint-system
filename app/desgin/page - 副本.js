...
export default function DesignerPage() {
  ...
  // 刷新 JSON 和画布
  const refreshCanvas = (updatedTemplate) => {
    const t = updatedTemplate || tpEntity;
    if (!t) return;
    const currentJson = t.getJson();
    setJsonText(JSON.stringify(currentJson, null, 2));
  };

  // 直接通过 JSON 变更添加字段（最稳定方案）
      ...
      <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start', width: '100%' }}>
        ...
        <div style={{ flex: 1, minWidth: 0, backgroundColor: '#ffffff', border: '1px solid #e2e8f0', padding: '16px', borderRadius: '12px', minHeight: '650px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div
            id="hiprint-designer"
            ref={designerRef}
            style={{ flex: 1, backgroundColor: '#f8fafc', border: '1px dashed #cbd5e1', borderRadius: '8px', padding: '20px', overflow: 'auto', display: 'flex', justifyContent: 'center' }}
          >
